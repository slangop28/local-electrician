...
import { Button, OTPLoginModal, Sidebar, ProfileDropdown, NotificationBell } from '@/components/ui';
import { useAuth } from '@/lib/AuthContext';
import { ActiveServiceWidget } from '@/components/ActiveServiceWidget';
...
export default function Home() {
  ...
  // Fetch active service for customers
  useEffect(() => {
    if (!isAuthenticated || !userProfile?.phone || userProfile.userType !== 'customer') return;
    ...
    const fetchService = async () => {
      try {
        ...
        console.error('Failed to fetch active service:', error);
      }
    };
  ...
  return (
    <main className="min-h-screen">
      ...
      <ActiveServiceWidget activeService={activeService} />
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              {isAuthenticated ? (
                <>
                  <NotificationBell
                    notifications={[]}
                    blink={activeService?.status === 'ACCEPTED'}
                  />
                  <ProfileDropdown />
                </>
              ) : (
      ...
      {/* Referral Promo Section - Premium & Compact */}
      <section className="relative py-8 bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 overflow-hidden">
        {/* Abstract Background Elements */}
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden opacity-20">
          <div className="absolute -top-24 -left-24 w-64 h-64 bg-cyan-500 rounded-full blur-[80px]" />
          <div className="absolute -bottom-24 -right-24 w-64 h-64 bg-purple-500 rounded-full blur-[80px]" />
        </div>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 shadow-2xl hover:shadow-cyan-500/10 transition-all duration-300 group">

            {/* Left: Text & Hook */}
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-xl flex items-center justify-center shadow-lg shadow-orange-500/20 group-hover:scale-110 transition-transform duration-300">
                <span className="text-2xl">🎁</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                  Refer & Earn <span className="text-yellow-400 font-extrabold">₹100</span>
                </h3>
                <p className="text-blue-200 text-sm">Know a skilled electrician? Invite them!</p>
              </div>
            </div>

            {/* Middle: Steps (Desktop only) */}
            <div className="hidden md:flex items-center gap-4 text-sm text-gray-300 opacity-80">
              <div className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-xs">1</span>
                <span>Refer Technician</span>
              </div>
              <span className="text-white/20">→</span>
              <div className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-xs">2</span>
                <span>They do 2 Services</span>
              </div>
              <span className="text-white/20">→</span>
              <div className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-xs">3</span>
                <span className="text-yellow-400 font-bold">Get ₹100</span>
              </div>
            </div>

            {/* Right: CTA */}
            <Link href="/electrician">
              <button className="px-6 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-white font-bold rounded-lg shadow-lg shadow-cyan-500/25 transition-all duration-300 transform group-hover:-translate-y-0.5 whitespace-nowrap">
                Refer Now
              </button>
            </Link>
          </div>
