 ...
 * JavaScript version (can run directly with node)
 */
...
async function runTest() {
    ...
    try {
        ...
        // ========================================
        log(colors.blue, 'STEP 1', 'Fetching available electricians from Supabase...');
        const electriciansRes = await request('GET', '/debug/electricians');
        ...
        if (!electriciansRes.success || !electriciansRes.electricians || electriciansRes.electricians.length === 0) {
            ...
            log(colors.yellow, 'INFO', 'Response:', JSON.stringify(electriciansRes, null, 2));
            process.exit(1);
        }

        const testElectrician = electriciansRes.electricians[0];
        log(colors.green, 'SUCCESS', `Found electrician: ${testElectrician.name} (${testElectrician.id})`);
        console.log('');
        ...
        const createRequestRes = await request('POST', '/request/create', {
            electricianId: testElectrician.id,
        ...
        const initialStatusRes = await request('GET', `/customer/active-request?customerId=${testCustomer.phone}`);
        ...
        if (initialRequest.electricianId === 'BROADCAST' || initialRequest.electricianId === testElectrician.id) {
        ...
        const acceptRes = await request('POST', '/electrician/update-request', {
            ...
            electricianPhone: testElectrician.phone,
        ...
        const updatedStatusRes = await request('GET', `/customer/active-request?customerId=${testCustomer.phone}`);
        ...
        // ========================================
        log(colors.blue, 'STEP 6', 'Waiting 6 seconds to test real-time sync & polling...');
        await new Promise(resolve => setTimeout(resolve, 6000));

        const realtimeStatusRes = await request('GET', `/customer/active-request?customerId=${testCustomer.phone}`);
